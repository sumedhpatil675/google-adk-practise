...
def update_interaction_history(session_service,app_name,user_id,session_id,entry):
    """ Addd a entry to the interaction history in state.
...
def add_user_query_to_history(session_history,app_name,user_id,session_id,query):
...
async def call_agent_async(runner,user_id,session_id,query):
    ...
    try:
        async for event in runner.run_async(
            ...
            # Capture the agent name from the event if avaiable
            if event.author:
